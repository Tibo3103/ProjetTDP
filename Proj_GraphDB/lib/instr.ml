...
let rec normalize_pattern act = function 
  ...
  | CompPattern (npt, rl, pt) -> 
      ...
      let instruction_rel    = IActOnRel (act, npt_var, rl, pt_var) in
      let instructions       = List.concat [npt_inst; pt_inst; [instruction_rel]] in
      (pt_var, instructions)
...
let normalize_clause = function
  ...
  | Delete pats -> 
      match pats with
      | DeleteNodes vars -> 
          List.map (fun v -> IDeleteNode v) vars
      | DeleteRels rels -> 
          List.map (fun (v1, lab, v2) -> IDeleteRel (v1, lab, v2)) rels
